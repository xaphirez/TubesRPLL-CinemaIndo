<div class="bg-secondary text-white">
    <!-- Hero section -->
    <section class="d-flex col-xl-12 py-4">
        <div class="carousel-tiem active ms-auto me-auto">
            <img src="<?= BASEURL; ?>/img/JW4.webp" alt="Jhon Wick 4" class="d-block w-100 py-5">
            <div class="carousel-caption d-none d-mb-block">
                <h5>bla bla bla</h5>
                <p> bla bla bla</p>
                <p><a href="#" class="btn btn-warning"> bla bla</a></p>
            </div>
        </div>
    </section>

    <!-- Now Playing -->
    <div class="card mb-3 bg-dark card align-items-center text-light mx-auto" style="max-width: 1100px;"
        id="Now_Playing">
        <h1>Now Playing</h1>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-4 p-3 py-4">
                <div class="card mb-4 box-shadow">
                    <img class="card-img-top" src="<?= BASEURL; ?>/img/TSMB.webp" alt="The Super Mario Bros"
                        height="300" class="img-fluid">
                    <div class="card-body">
                        <h5 class="card-title">The Super Mario Bros. Movie</h5><br>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">5 April 2023</small>
                            <a href="#" class="btn btn-sm btn-outline-secondary">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4 p-3 py-4">
                <div class="card mb-4 box-shadow">
                    <img class="card-img-top" src="<?= BASEURL; ?>/img/SNT.webp" alt="Suzume no Tojimari" height="300"
                        class="img-fluid">
                    <div class="card-body">
                        <h5 class="card-title">Suzume no Tojimari</h5><br>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">8 Maret 2023</small>
                            <a href="#" class="btn btn-sm btn-outline-secondary">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4 p-3 py-4">
                <div class="card mb-4 box-shadow">
                    <img class="card-img-top" src="<?= BASEURL; ?>/img/WM.webp" alt="Waktu Maghrib" height="300"
                        class="img-fluid">
                    <div class="card-body">
                        <h5 class="card-title">Waktu Maghrib</h5><br>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">9 Februari 2023</small>
                            <a href="#" class="btn btn-sm btn-outline-secondary">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4 p-3 py-4">
                <div class="card mb-4 box-shadow">
                    <img class="card-img-top" src="<?= BASEURL; ?>/img/DD.jpg"
                        alt="Dungeons & Dragons: Honor Among Thieves" height="300" class="img-fluid">
                    <div class="card-body">
                        <h5 class="card-title">Dungeons & Dragons: Honor Among Thieves</h5>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">28 Maret 2023</small>
                            <a href="#" class="btn btn-sm btn-outline-secondary">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4 p-3 py-4">
                <div class="card mb-4 box-shadow">
                    <img class="card-img-top" src="<?= BASEURL; ?>/img/Shazam.jpg" alt="Shazam! Fury of the Gods"
                        height="300" class="img-fluid">
                    <div class="card-body">
                        <h5 class="card-title">Shazam! Fury of the Gods</h5><br>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">15 Maret 2023</small>
                            <a href="#" class="btn btn-sm btn-outline-secondary">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4 p-3 py-4">
                <div class="card mb-4 box-shadow">
                    <img class="card-img-top" src="<?= BASEURL; ?>/img/Tulah.jpg" alt="Tulah 613" height="300"
                        class="img-fluid">
                    <div class="card-body">
                        <h5 class="card-title">Tulah 613</h5><br>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">30 Maret 2023</small>
                            <a href="#" class="btn btn-sm btn-outline-secondary">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Up Coming -->
    <div class="card mb-3 bg-dark card align-items-center text-light mx-auto" style="max-width: 1100px;"
        id="Coming_Soon">
        <h1>Coming Soon</h1>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-4 p-3 py-4">
                <div class="card mb-4 box-shadow">
                    <img class="card-img-top" src="<?= BASEURL; ?>/img/GOTG3.webp" alt="Guardian Of The Galaxy Volume 3"
                        height="300" class="img-fluid">
                    <div class="card-body">
                        <h5 class="card-title">Guardian Of The Galaxy Volume 3</h5><br>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">5 Mei 2023</small>
                            <a href="#" class="btn btn-sm btn-outline-secondary">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4 p-3 py-4">
                <div class="card mb-4 box-shadow">
                    <img class="card-img-top" src="<?= BASEURL; ?>/img/spiderman.webp"
                        alt="Spider-Man:Across The Spider-Verse" height="300" class="img-fluid">
                    <div class="card-body">
                        <h5 class="card-title">Spider-Man:Across The Spider-Verse</h5><br>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">2 Juni 2023</small>
                            <a href="#" class="btn btn-sm btn-outline-secondary">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4 p-3 py-4">
                <div class="card mb-4 box-shadow">
                    <img class="card-img-top" src="<?= BASEURL; ?>/img/fastX.webp" alt="Fast X" height="300"
                        class="img-fluid">
                    <div class="card-body">
                        <h5 class="card-title">Fast X</h5><br><br>
                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">19 Mei 2023</small>
                            <a href="#" class="btn btn-sm btn-outline-secondary">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>